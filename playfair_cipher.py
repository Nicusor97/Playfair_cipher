...
class Playfair:
    ...
    @staticmethod
    def build_table(key):
        return ''.join(sorted(set(key), key=lambda x: key.index(x)))+''.join([ch for ch in ALPHABET if not (ch in key)
                                                                              and ch != 'J'])
    ...
    def substitution(message, table, *, mode):
        ...
        list_pos = [[[table.index(elem[0])//5, table.index(elem[0]) % 5],
                    [table.index(elem[1])//5, table.index(elem[1]) % 5]] for elem in list_message]
        list_pos2 = []
...
def main():
    ...
    key = Playfair.build_table('daniel'.upper())
    encrypt_text = Playfair.encrypt('anaaremereverzi'.upper(), key)
    print("After encrypt text is : %s" % encrypt_text)
    decrypt_test = Playfair.decrypt(encrypt_text, key)
    print("After decrypt text is : %s" % decrypt_test)
